...
static bool isAtEnd(const Lexer *lx) { return lx->pos >= lx->len; }
static char peek(const Lexer *lx) { return (lx->pos < lx->len) ? lx->src[lx->pos] : '\0'; }
static char next(const Lexer *lx) { return ((lx->pos + 1) < lx->len) ? lx->src[lx->pos + 1] : '\0'; }
...
static TokenKind check_keyword(const char *text, size_t len)
{
    ...
    for (size_t i = 0; i < sizeof kws / sizeof *kws; i++)
        if (slice_eq(text, len, kws[i].word))
            return kws[i].kind;
...
static Token scan_symbol(Lexer *lx)
{
    ...
    {
    case '.':
        return make_token(lx, Dot);
    case ',':
        return make_token(lx, Comma);
    case ';':
        return make_token(lx, Semicolon);
    case '(':
        return make_token(lx, LParen);
    case ')':
        return make_token(lx, RParen);
    case '{':
        return make_token(lx, LBrace);
    case '}':
        return make_token(lx, RBrace);
    case '+':
        if (peek(lx) == '=')
        {
            consume(lx);
    ...
    case '[':
        return make_token(lx, LBracket);
    case ']':
        return make_token(lx, RBracket);
    ...
    while (!isAtEnd(lx) && peek(lx) != '"')
    {
        ...
        if (peek(lx) == '\\')
        {
            ...
            if (!is_valid_escape(e))
                tarm_error_at(lx->diag, lx->line, lx->col,
                              "sequência de escape inválida: '\\%c'", e);
            consume(lx);
            continue;
        }
        consume(lx);
...
static Token qualify_next_token(Lexer *lx)
{
    ...
    if (isAtEnd(lx))
        return make_token(lx, EndOfFile);
...
void tarm_lexer_dump_tokens(FILE *out, const TokenList *list)
{
    for (size_t i = 0; i < list->count; i++)
    {
        ...
        fprintf(out, "%3zu  %-16s %2u:%-3u '%.*s'\n",
                i, tarm_token_kind_name(t.kind), t.line, t.col, (int)t.len, t.start);
    }
}