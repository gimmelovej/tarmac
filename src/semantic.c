                      ...
                      bool *reported)
{
    ...
    if (from.type == expected.type)
        return true;

    CastKind ck;
    if (!find_implicit_cast(from.type, expected.type, &ck))
        return false;
...
static DataType check_binary_op(SemanticAnalyzer *an, Expr *e)
{
    ...
    else
    {
        tarm_error_at(an->diag, e->line, e->col,
                      ...
                      tarm_semantic_type_name(lt.type), tarm_semantic_type_name(rt.type));
        return tarm_datatype_of(Void);
    }
...
static DataType check_call(SemanticAnalyzer *an, Expr *e)
{
    ...
    if (!sig)
    {
        ...
        if (is_method)
            tarm_error_at(an->diag, e->line, e->col,
                          "'%s' não tem método '%.*s'",
                          tarm_semantic_type_name(receiver.type),
    ...
    for (size_t i = first; i < e->as.call.arg_count; i++)
    {
        ...
        if (mask & TARM_TYPE_BIT(actual.type))
            continue;
        ...
        for (int t = 0; t <= (int)Void && !coerced && !reported; t++)
            if (mask & TARM_TYPE_BIT((BaseType)t))
                coerced = coerce_to(an, &e->as.call.args[i], actual, tarm_datatype_of((BaseType)t), &reported);
...
static DataType check_expr(SemanticAnalyzer *an, Expr **slot)
{
    ...
    // --- literais ---------------------------------------------------------------------------
    case ExprInteger:
        result = tarm_datatype_of(Int);
        break;
    case ExprFloat:
        result = tarm_datatype_of(Float);
        break;
    case ExprBool:
        result = tarm_datatype_of(Bool);
        break;
    case ExprString:
        result = tarm_datatype_of(String);
        break;
    case ExprChar:
        result = tarm_datatype_of(Char);
        break;
    ...
    case ExprVarDecl:
    {
        ...
        if (init && declared.is_array && init->kind == ExprArrayLit) {
            if (init->as.array_lit.count > declared.array_len) tarm_error_at(an->diag, init->line, init->col, 
                "quantidade de elementos acima do reservado em: '%.*s", (int)target->as.identifier.len,
                target->as.identifier.name);
        ...
        if (e->as.var_decl.frame == Global)
            ok = tarm_symbol_table_declare_global(an->symbols,
                                                  target->as.identifier.name, target->as.identifier.len,
                                                  declared, 0, NULL);
        else
